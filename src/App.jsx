...
• weather and weather_window: if cities are in very different climates (mountain vs coast vs desert), call this out in weather_window AND give per-day weather that reflects the city's actual climate for that day.` : "";
    ...
    //                      hit cache (~10% of input-token cost).
    //   • dynamicPreamble — per-trip identity line, conditional constraint
    //                      blocks (private driver / private tour / etc.),
    //                      destination facts, exact day count, and the three
    //                      destination-filtered fact blocks (marquee sights,
    //                      airport mappings, route truth).
    //
    // The build call site assembles them as:
    //     [staticRules block (cached)] + [dynamicPreamble block] + user message
    //
    // This is a REORDER vs. the previous prompt: trip-specific overrides now
    // appear AFTER the general rulebook. Anthropic prefix caching requires
    // the cached block to come FIRST. We accept this reorder because:
    //   (a) it mirrors a natural "rules → specific overrides" structure,
    //   (b) every override block is self-contained and re-asserts itself
    //       (e.g. PRIVATE DRIVER — HARD RULE), so position-in-prompt is
    //       not load-bearing.
    // -------------------------------------------------------------------
    ...
    const staticRules = `FIELD EMISSION ORDER — CRITICAL:
Write the tool input in this exact order: destination, meta, [cities,] days, logistics, flags, planb, snobs, tonight. The [cities,] field appears only on multi-city trips — see the per-trip preamble below.
...
TONE: Insider, opinionated, specific. Real names, real dishes, real neighborhood detail. Avoid travel-blog vagueness.`;
    ...
    const _multiCityFieldOrder = isMultiCity
      ? `\nFIELD EMISSION ORDER OVERRIDE: include 'cities,' in the field order (the [cities,] bracketed slot in the static rulebook).`
      : "";

    const dynamicPreamble = `You are a luxury travel planner. Call the submit_trip_plan tool exactly once with the finalized plan. Do not emit any prose — only the tool call.${trainRuleBlock}${privateDriverBlock}${privateTourBlock}${skipTheLineBlock}
${_destinationFactsBlock}

PER-TRIP REQUIREMENTS (these are the trip-specific values referenced by the static rulebook above):
