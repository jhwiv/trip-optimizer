...
// --- Flight time single-source-of-truth (RCA bugs A / B / C) -----------------
// The flight resolver (flightResolver.js) and the #133 timezone fix overwrite
// the STRUCTURED flight times (item.flight.depart_time / .arrive_time) from the
// live schedule, but never the model-stamped item.time. Per CLAUDE.md
// ("time math … COMPUTED IN CODE, never generated by the model") the PDF must
// treat the structured flight object as the single source of truth for both
// the schedule stamp and the chronological sort — the flight CARD already
// renders from fl.depart_time/arrive_time, so reading the same field for the
// stamp/sort keeps them from ever disagreeing.

// The schedule-stamp time for a day item. For a flight item this is the
// resolved departure time (the instant the event begins), never the possibly-
// stale item.time; falls back to item.time only when the flight carries no
// resolved departure (RCA bug A).
export function flightStampTime(item) {
  const fl = item?.flight;
  if (fl) {
    const dep = safe(fl.depart_time).trim();
    if (dep) return dep;
  }
...
function renderItem(cur, item, isLast, itemPhotos = {}, dayCity = "") {
  ...
  // Always render times in 12h AM/PM (UI parity). Flight items read the
  // resolved structured departure time (flightStampTime) rather than the
  // possibly-stale model item.time, and show a single instant — the arrival is
  // carried on the flight card, so a depart–arrive range here would be
  // misleading for an overnight leg (RCA bug A).
  const time = to12h(flightStampTime(item));
  const endTime = item.flight ? "" : to12h(safe(item.end_time));
