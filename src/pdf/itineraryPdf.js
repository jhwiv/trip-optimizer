...
// Page geometry — US Letter portrait, generous margins for a printable feel.
const PAGE = {
...
function renderCover(cur, data, inputs, opts) {
  ...
  // Compact top — leave just enough white space to feel airy without
  // pushing the summary onto a second page.
  cur.space(14);
  ...
  // Meta line
  if (meta) {
    cur.text(meta, { font: FONT.sans, style: "normal", size: 11.5, color: COLOR.inkSoft, leading: 1.4 });
  }
  ...
  cur.space(6);
  cur.rule({ color: COLOR.rule, space: 1 });
...
function renderDay(cur, day, index) {
  ...
  cur.ensureSpace(40); // need real room before starting a day
  ...
  cur.space(2);
  cur.rule({ color: COLOR.rule, space: 1.5 });
  cur.space(2);
...
function renderItem(cur, item, isLast) {
  ...
  const endTime = safe(item.end_time);
  ...
  if (cur.state.y < itemTop + 12) cur.state.y = itemTop + 12;
}
...
function renderFlightBlock(cur, fl, x, maxW) {
  // Single combined headline line: "United UA 1234 · EWR 08:45 → ABQ 11:20 · 4h 35m"
  const headline = [
    [fl.carrier, fl.flight_number].filter(Boolean).join(" "),
    [fl.from_airport, fl.depart_time].filter(Boolean).join(" "),
    "→",
    [fl.to_airport, fl.arrive_time].filter(Boolean).join(" "),
...
function renderHotelBlock(cur, h, x, maxW) {
  ...
  const ci = [h.check_in_time ? `In ${h.check_in_time}` : "", h.check_out_time ? `Out ${h.check_out_time}` : ""].filter(Boolean).join("  ·  ");
