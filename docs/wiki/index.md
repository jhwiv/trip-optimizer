...
- `learnings/2026-08-04.md` — a real 14-night 5-city build's PDF export blocked by a false-positive `DAY_CITY_DISCONTINUITY` (fixed in `dayContinuityCheck.js`, confirmed live via a browser `download` event before/after); a Save-trip-button false-positive "Saved" on a storage write failure; stage 1 of an itinerary-build → deployable-site pipeline (`scripts/publish-trip-site.mjs`), informed by cloning and reading this account's own three real trip-site repos rather than guessing; and a mount-time build-resume effect that could silently hijack the app into a token-consuming rebuild instead of showing the home screen or a saved trip — now an explicit Resume/Discard choice (`CLAUDE.md`'s "KNOWN FAILURE MODE #4")
