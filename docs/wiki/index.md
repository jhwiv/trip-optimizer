...
- `learnings/2026-08-04.md` — a real 14-night 5-city build's PDF export blocked by a false-positive `DAY_CITY_DISCONTINUITY`: an arrow-formatted transit-day city label got fed into the canonical city-resolution list and won a substring match against the real destination city, compounded by a comma-qualifier in a Flight's route text and a raw (unresolved) string comparison in the discontinuity check itself; three fixes landed together in `dayContinuityCheck.js`, confirmed live via a browser `download` event before/after
