...
| `DUPLICATE_VENUE_SAME_DAY` | warn | The same restaurant was emitted twice for the same meal type on the same day (`applyQualityLayer` §1) — a generation duplication bug, not a second meal. The duplicate item is auto-removed (kept: the first, usually more complete, occurrence); the flag is for visibility | no |
